...
private void printProgram( byte[] program )
{
    size_t pos;
...
private void enumerateStates( byte[] program, out size_t numStates )
{
    size_t pos;
    while( pos < program.length )
    {
...
public struct RegexSingleMatch(String)
{
    ...
    /++
     Get the position in the string of the start of the  match
     +/
    ...
    /++
     Get the position in the string of the end of the  match
     +/
...
public struct RegexMatch(String,RegexEngine,RegexSingleMatchType)
{
    ...
    /++
     Return portion of the string that matches the regular expression
     +/
    String hit()
    {
    ...
    /++
     Return the portion of the string after the match
     +/
    String post()
    {
...
public struct Captures(String,RegexSingleMatchType)
{
    ...
    /++
     Return the subexpression corresponding to the given index.
    ...
    @property size_t length()
    {
        ...
        int i = 0;
        while( i <_match.length )
        {
            if ( _match._captures[i] != size_t.max )
                i += 2;
        }
                         ...
                         String state_s, size_t[] state_captures, ExecState!(String)* execState )
    {
        ...
        for( ;; )
        {
            ...
            case REInst.Char:
                ...
                if ( (cast(InstChar*)inst).c != thisChar )
                    return 0;
            ...
            case REInst.BOL:
                if ( !isNewLineChar( state_s, getPrevPos( state_s, state_sPos ) ) )
                    return 0;
            ...
            case REInst.WordBoundary:
                ...
                size_t prevSPos = getPrevPos( state_s, state_sPos );
       
                if( isWordChar( state_s, prevSPos ) &&
                    !isWordChar( state_s, state_sPos ) )
                    result = true;
                else if ( !isWordChar( state_s, prevSPos ) &&
                          isWordChar( state_s, state_sPos ) )
                    result = true;
